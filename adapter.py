...
def decode_push():
    global r
    inc_dict=ast.literal_eval(r.lpop("MQ-side-messages").decode('utf-8'))
...
#Proto encode and publish to appropriate topic
...
def NSSub_onMessage(client, userdata, msg):
    ...
    r.rpush("MQ-side-messages", mq-dic)
    decode_push.delay()

#-------------------------------------------------------------------------------------------------------------------------

#To import proto files to the code dynamically
...
def server():
    ...
    while True:
        ...
        itemId = itemEntry["id"]
        modules[itemId] = {}
	#Import from files for decoding and to files for encoding when new device is regsitered
        try:
