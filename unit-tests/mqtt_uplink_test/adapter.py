...
def avg():
    ...
    avg_time=(sumg/countg)
    print("Average time for", avg_time, "seconds")
    gr=open('bps.dat', 'a')
    gr.write('10\t\t%f\n' %avg_time)
    gr.close()
#----------------------------------------------------------------------------------------------------#
...
#            print("Published", mw_message)
    ...
    redConn.rpush("mq-times", (end-start))
...
def NSSub_onDisconnect(client, userdata, rc):
    pass
...
nsSubParams["onDisconnect"] = NSSub_onDisconnect
